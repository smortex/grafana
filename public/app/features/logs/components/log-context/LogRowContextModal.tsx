...
import { Icon, Button, LoadingBar, Modal, useTheme2, RadioButtonGroup } from '@grafana/ui';
...
export const LogRowContextModal: React.FunctionComponent<LogRowContextModalProps> = ({
  ...
  const [loadingMore, setLoadingMore] = useState<'top' | 'bottom' | null>(null);
  ...
  useEffect(() => {
    ...
    if (top != null && !top.disabled) {
      observer.observe(top);
    }

    if (bottom != null && !bottom.disabled) {
  ...
  return (
    ...
    >
      ...
      <div className={cx(styles.flexRow, styles.paddingBottom)}>
        <RadioButtonGroup
          options={[
            { label: 'auto-load', value: 'auto' },
            { label: 'manual-load', value: 'manual' },
          ]}
          value={shouldAutoLoadMore ? 'auto' : 'manual'}
          onChange={(v) => {
            setShouldAutoLoadMore(v === 'auto');
          }}
        />
              ...
              <td className={styles.noMarginBottom}>
                <>
                  <button
                    ...
                    disabled={loadingMore != null || !context.hasTop}
                  >
                    {loadingMore == null ? 'load more' : 'loading...'}
                  </button>
                  <LogRows
              ...
              <td>
                <>
                  ...
                  <button
                    ...
                    disabled={loadingMore != null || !context.hasBottom}
                  >
                    {loadingMore == null ? 'load more' : 'loading...'}
                  </button>
                </>
